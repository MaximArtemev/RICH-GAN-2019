...
data_dir = 'content/drive/My Drive/data/data_calibsample/'
...
def get_tf_dataset(dataset, batch_size):
    ...
    return suffled_ds.batch(batch_size).prefetch(1).compat.v1.make_one_shot_iterator().get_next()

def scale_pandas(dataframe, scaler):
    return pd.DataFrame(scaler.transform(dataframe.values), columns=dataframe.columns)


def get_all_particles_dataset(dtype=None, log=False, n_quantiles=100000, list_particles=list_particles):
    data_train_all = []
    data_val_all = []
    scaler_all = {}
