...
from nf.inner_nets import FCNN
...
class ConditionalNSF_AR(nn.Module):
    def __init__(self, dim, cond_dim, K=5, B=3, hidden_dim=8, base_network=FCNN):
        super().__init__()
        self.dim = dim
        ...
        for i in range(1, dim):
            self.layers += [base_network(i + self.cond_dim, 3 * K - 1, hidden_dim)]
        self.reset_parameters()
